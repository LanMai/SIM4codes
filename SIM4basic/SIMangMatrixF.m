function [ MAp, fSp, NoiseStd ] = SIMangMatrixF( ModFac, Kotf, MaskPetals,...
    S1aTnoisy, sAo, PSFe, OTFo, doubleSize, DIo )

% DIo: is used here for verifying the correctness of matrix MAp
...
fDp = fDIo(IndexMaskPetals); % used here for verifying the correctness of matrix MAp
...
parfor u = 1:Zcount
    ...
    MAp(u,:) = IndexZo(u,2).*fsAo1(IndexMaskPetals)./normalizingW^2;
end

MAp = conj(MAp);

%% for verification of MAp
%{
figure;
hold on
plot(real(fSp),'go-','LineWidth',2,'MarkerSize',8)
plot(real(MAp*fDp),'o--','LineWidth',2,'MarkerSize',6)
grid on
box on

figure;
hold on
plot(imag(fSp),'go-','LineWidth',2,'MarkerSize',8)
plot(imag(MAp*fDp),'o--','LineWidth',2,'MarkerSize',6)
grid on
box on
kk
%}
